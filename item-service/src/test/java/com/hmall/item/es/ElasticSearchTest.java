...
public class ElasticSearchTest {
    ...
    @Test
    void testMatchAll() throws IOException {//简单查询
        // 1.创建Request对象
    ...
    @Test
    void testSearch() throws IOException {//复合查询
        // 1.创建Request对象
        SearchRequest request = new SearchRequest("items");
        // 2.组织DSL参数
        request.source()
    ...
    @Test
    void testSortAndPage() throws IOException {//查询中进行排序和分页
        // 0.模拟前端传递的分页参数
    ...
    @Test
    void testHighlight() throws IOException {//高亮分词查询
        // 1.创建Request对象
        SearchRequest request = new SearchRequest("items");
        // 2.组织DSL参数
        // 2.1.query条件
    ...
    @Test
    void testAgg() throws IOException {//桶聚合相当于group by
        // 1.创建Request对象
        SearchRequest request = new SearchRequest("items");
        // 2.组织DSL参数
        // 2.1.分页
        ...
        // 4.1.根据聚合名称获取对应的聚合
        Terms brandTerms = aggregations.get(brandAggName); //特别注意，这里接收的函数应该保持与传入时的AggregationBuilders.terms一致，而不是默认使用它的父接口Aggregation
        // 4.2.获取buckets
